...
#### Used for
- Testing independence in a 2×2 contingency table without conditioning on all marginal totals (unconditional exact test).  
- **Real-world example:** Determining if a new pain medication reduces adverse events compared to placebo when sample size is small and margins are not fixed.

#### Assumptions
- Observations are independent.  
- Data form a 2×2 table (binary outcome × binary exposure).  
- Only one margin (row or column) may be treated as fixed; the other is free (unconditional design).  

#### Strengths
- Generally more powerful than Fisher’s exact test for 2×2 tables because it does not condition on both margins.  
- Provides an exact p-value without relying on large-sample approximations.  
- Particularly advantageous when the margins are not fixed by design.

#### Weaknesses
- Computationally more intensive than Fisher’s exact test.  
- Less widely available in standard software (requires specialized packages such as `exact2x2` or `Barnard`).  
- Interpretation of the unconditional approach can be less intuitive if one is accustomed to conditional tests.

#### Example

##### Hypothesis
- **Null hypothesis (H₀):** The probability of an adverse event is the same for medication and placebo (no association).  
- **Alternative hypothesis (H₁):** The probability of an adverse event differs between medication and placebo.

```{r barnards_exact_test}
# Install and load the exact2x2 package if necessary:
...
ctab <- matrix(c(2, 8,
               ...
               dimnames = list(
                 ...
                 AdverseEvent  = c("Yes", "No")
               ))

# Perform Barnard’s exact test (unconditional):
barnard_result <- barnard.test(ctab, test = "score")  # "score" is the default ordering
...
Interpretation:
Barnard’s test reports an unconditional test statistic and an exact p-value. 
Suppose the output shows p-value = `r signif(barnard_result$p.value, 3)`. We
`r if(barnard_result$p.value < 0.05) "reject the null hypothesis" else "fail to reject the null hypothesis"`.
This means there is
`r if(barnard_result$p.value < 0.05) "evidence that the probability of an adverse event differs between medication and placebo." else "no evidence to conclude a difference in adverse event rates between medication and placebo."`
