...
# Observed counts of M&M colours:
...
with a two-sided p-value of `r signif(two_sided_p_value, 3)`. We
`r if(two_sided_p_value < 0.05) "reject the null hypothesis" else "fail to reject the null hypothesis"`.
Thus, there is
`r if(two_sided_p_value < 0.05) "evidence that the average diameter differs from 5.00 cm." else "no evidence to conclude a difference from the specified diameter of 5.00 cm."`

We can similarly reject the hypothesis that the average diameter is larger (p = `r p_value_larger_mu`)
or that it is smaller (p = `r p_value_smaller_mu`)
...
```{r ekstraher_levene_resultater,  echo = FALSE}
...
Levene's test yields an F-statistic of `r round(f_stat, 2)` with a p-value of `r signif(p_val, 3)`. We `r if(p_val < 0.05) "reject the null hypothesis" else "fail to reject the null hypothesis"`. This means there is `r if(p_val < 0.05) "evidence that at least one clinic's blood pressure variability differs from the others." else "no evidence of differing variances across clinics."` 
