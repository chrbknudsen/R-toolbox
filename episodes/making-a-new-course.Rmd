...
- How can a new course site be created from the R-toolbox template?
- Which configuration, episode, dependency, and citation files must be changed?
- How can the course be built, published, updated, and released?
...
- Configure the title, source, episode order, and other project metadata.
- Remove unused episodes and dependencies.
- Build and check the lesson before enabling GitHub Pages.
- Update citation metadata and create a documented release.
...
Note that this should work. But that you may have to rebuild the `renv` cache on Github by running the workflow "O3 Maintain: Apply Package Cache" manually.
