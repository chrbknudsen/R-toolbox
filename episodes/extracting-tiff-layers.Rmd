...
popular format for storing satelite imagery, where each layer contains different exposures of the same geographical area. Layers can be split by colour, where a layer containing the green channel of an rgb-image, can make it easier to
identify vegetation. 
...
Original Landsat images can be accessed from [USGS](https://www.usgs.gov/landsat-missions). Note that this example is coloured with 
false colours, and stacked into a single file. Most data from USGS are provided in individual images for each "band" of the spectrum.
