...
haem_min <- min(haem)
haem_max <- max(haem)
haem_range <- range(haem)
haem_mean <- round(mean(unlist(haem)),2)
haem_median <- round(median(unlist(haem)),2)
haem_sd <- sd(unlist(haem))
haem_var <- var(unlist(haem))
gt_table
```
...
A reasonable place to start would be reporting the minimum: `r haem_min` and 
maximum: `r haem_max`, and, by implication, the range: `r haem_range`

Those are easy to find in this table, using the `min()`, `max()´, and `range()`
functions respectively.

The mean, or average would also be relevant. In this case it is `r haem_mean`.

Similarly the median: `r haem_median`
...
The standard deviataion: `r haem_sd` and variance: `r haem_var` describes this,
