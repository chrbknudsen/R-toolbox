...
We can make the previous plot, just coloured by Sex (Sex is recorded as
0's and 1's). We need to coeerce Sex to a categorical variable using `factor()`:

```{r new_scatter_plot}
#| eval: false
fev |> ggplot(aes(x=Hgt, y = FEV, colour = factor(Sex))) +
  geom_point() +
  geom_smooth(method = "lm")
```
...
```
We may need to play around with background colour to even be able to see the
small values.
